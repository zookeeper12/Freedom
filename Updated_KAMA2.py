...
###### USER PARAMETERS ######
token = 'a46809342f7ba500f343e60c5f1d10c20b51cd3a'
symbol = 'US30'
...
def Update():
    print(str(dt.datetime.now()) + "	 " + timeframe + " Previous Bar Closed - Running Update Function...")

    # Calculate Indicators
    #iFastSMA = sma(pricedata['bidclose'], fast_sma_periods)
    #iSlowSMA = sma(pricedata['bidclose'], slow_sma_periods)
    #iKama = talib.KAMA(pricedata['bidclose'], timeperiod=30)
    pricedata['KAMA'] = talib.KAMA(pricedata['bidclose'], timeperiod=30)
    pricedata['position'] = np.where(pricedata['KAMA'].shift(-1) > pricedata['KAMA'], 1, -1)


    # Print Price/Indicators
    print("Close Price: " + str(pricedata['bidclose'][len(pricedata) - 1]))
    print("Close KAMA: " + str(pricedata['KAMA'][len(pricedata) - 1]))
    print("New position: " + str(pricedata['position'][len(pricedata) - 1]))
    #print("  KAMA: " + str(iKama[len(iKama) - 1]))
    #print("P_KAMA: " + str(iKama[len(iKama) - 2]))
    #a = iKama[len(iKama) - 1].round(2)
    #b = iKama[len(iKama) - 2].round(2)
    #print("KAMA " + str(a))
    #print("PAMA " + str(b))
    ...
    if pricedata['position'][len(pricedata) - 1] == 1:
        print("	  BUY SIGNAL!")
    ...
    pricedata.drop(['position'], axis=1, inplace=True)
    print("----------------------------------------------------------------------")
    ...
    for position in openpositions:
        if position['currency'] == symbol:
            if BuySell is None or position['isBuy'] == isbuy:
                ...
                try:
                    closetrade = con.close_trade(trade_id=position['tradeId'], amount=amount)
                except:
